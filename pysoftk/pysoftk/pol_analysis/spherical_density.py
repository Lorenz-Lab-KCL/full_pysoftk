...
class spherical_density(MDA_input):
    ...
    def dist_com(self, u, atom_sel_micelle, micelle_positions, atom_selection):
       ...
       atom_sel_micelle : mda.object
        ...
        atom_selection : list
            list with the names of the atom selection for the density calculation
       ...
       distance_com: np.array
        ...
        atom_group = micelle.select_atoms('name '+str(" ".join(atom_selection)))
    

        #distance_COM = cdist(atom_group.positions, micelle_com, 'euclidean')

        distance_COM=MDAnalysis.analysis.distances.distance_array(atom_group.positions, micelle_com)
    ...
    def run_density_calc(self, micelle_sel_type, whole_micelle_selection, micelle_positions, group_atom_selection, maxbin=200, minbin=0, step=0.1):
       ...
       -----------
        ...
        group_atom_selection : str
            list with the names of the polymer species chosen for the density calculation
       ...
       density: np.array
        ...
        group_atom_selection_f = [group_atom_selection]*len(micelle_positions)


        distance_COM_total = map(self.dist_com, us, whole_micelle_selection_f, micelle_positions, group_atom_selection_f)
